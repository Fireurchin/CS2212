...
public class MainUI extends JFrame {
	...
	private MainUI() {
		...
		/*
		 * UC3: Selecting a country to fetch
		 * 		- All countries added to dropdown
		 * 		- TODO restrict countries based on selected analyses
		 */
		...
		//Add country names to GUI

		for (int i = 0; i < countries.size(); i++) 
        {
			countriesNames.add(countries.list().get(i));
		}
		
		countriesNames.sort(null);
		
		
		//INITIALIZE COUNTRY DROPDOWN LIST
		...
		countriesList.addItemListener(new ItemListener(){
            public void itemStateChanged(ItemEvent e){
            	if(e.getStateChange() == ItemEvent.SELECTED) {
                    String  a = countriesList.getSelectedItem().toString();
                    selectedCountry = a;
                    System.out.print(a); 
                }
            }
        });
		countriesList.setSelectedItem("Afghanistan");
		//selectedCountry = (String) countriesList.getSelectedItem();
		...
		int validStartYear = 1962;
		int validEndYear = 2021;
		...
		for (int i = validEndYear; i >= validStartYear; i--) 
		{
			years.add("" + i);
		}
		
		
		//INITIALIZE YEARS DROPDOWN LIST
		final JComboBox<String> fromList = new JComboBox<String>(years);
		fromList.addItemListener(new ItemListener(){
            public void itemStateChanged(ItemEvent e){
            	if(e.getStateChange() == ItemEvent.SELECTED) {
                    String  a = fromList.getSelectedItem().toString();
                    startYear = a;
                    System.out.print(a); 
                }
            }
        });
		fromList.setSelectedItem("1962");
		...
		toList.addItemListener(new ItemListener(){
            public void itemStateChanged(ItemEvent e){
            	if(e.getStateChange() == ItemEvent.SELECTED) {
                    String  a = toList.getSelectedItem().toString();
                    endYear = a;
                    System.out.print(a); 
                }
            }
        });
		fromList.setSelectedItem("2021");
		...
		viewsList.addItemListener(new ItemListener(){
            public void itemStateChanged(ItemEvent e){
            	if(e.getStateChange() == ItemEvent.SELECTED) {
                    String  a = viewsList.getSelectedItem().toString();
                    selectedView = a;
                    System.out.print(a); 
                }
            }
        });
		viewsList.setSelectedItem("Bar Graph");
		...
		methodsNames.add("Total Population vs CO2 Emissions");
		methodsNames.add("Hospital Beds");
		methodsNames.add("GDP per capita (current US$)");
		methodsNames.add("Education vs Health - (% of GDP)");
		methodsNames.add("Air Pollution vs Forest Area");
		methodsNames.add("Health Expidenture (current US$)");
		methodsNames.add("Forest Area vs Energy Use");
		...
		methodsList.addItemListener(new ItemListener(){
            public void itemStateChanged(ItemEvent e){
            	if(e.getStateChange() == ItemEvent.SELECTED) {
                    String  a = methodsList.getSelectedItem().toString();
                    selectedMethod = a;
                    System.out.print(a); 
                }
            }
        });
		methodsList.setSelectedItem("Forest Area");
		...
		/*recalculate.addActionListener(new ActionListener()
		{
		  ...
		  {
			  Request request = new Request(selectedCountry, selectedMethod, data, startYear, endYear, selectedView);
		  }
		});*/
	...
	private void createBar(JPanel west) {
		...
		plot.setDomainAxis(domainAxis);
		plot.setRangeAxis(new NumberAxis("jbnj"));
